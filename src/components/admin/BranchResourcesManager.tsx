...
const BranchResourcesManager = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const {
        data: branchResourceTypesData,
        error: branchResourceTypesError
      } = await supabase.from('branch_resource_types').select(`
          *,
  ...
  const handleAddResource = async () => {
    try {
      ...
      const existingResource = branchResourceTypes.find(r => r.branch_id === selectedBranch.id && r.resource_type_id === resourceTypeId);
      if (existingResource) {
        toast.error('هذا المورد موجود بالفعل لهذا الفرع');
        return;
      }
      const {
        data: newBranchResource,
        error: addError
      } = await supabase.from('branch_resource_types').insert({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editMode && selectedResourceType) {
        const resourceToEdit = branchResourceTypes.find(r => r.branch_id === selectedBranch.id && r.resource_type_id === selectedResourceType);
        if (!resourceToEdit) {
        ...
        const {
          error
        } = await supabase.from('branch_resource_types').update({
          available: formData.available,
          in_use: formData.in_use
        }).eq('id', resourceToEdit.id);
        if (error) throw error;
        setBranchResourceTypes(prevResources => prevResources.map(r => r.id === resourceToEdit.id ? {
          ...r,
          available: formData.available,
          in_use: formData.in_use
        } : r));
  ...
  const handleEditResource = (branchId: string, resourceTypeId: string) => {
    const resource = branchResourceTypes.find(r => r.branch_id === branchId && r.resource_type_id === resourceTypeId);
    if (resource) {
  ...
  const handleDeleteResource = async (resourceId: string) => {
    if (window.confirm('هل أنت متأكد من حذف هذا المورد من الفرع؟')) {
      try {
        const {
          error
        } = await supabase.from('branch_resource_types').delete().eq('id', resourceId);
        if (error) throw error;
        setBranchResourceTypes(prevResources => prevResources.filter(r => r.id !== resourceId));
  ...
  const BranchResourceRow = ({
    resource
  }: {
    resource: BranchResourceType;
      ...
      <div className="flex gap-4">
        ...
        <div className="flex items-center space-x-1">
          <Button variant="ghost" size="icon" onClick={() => handleEditResource(resource.branch_id, resource.resource_type_id)} title="تعديل">
            <Edit className="h-4 w-4 text-blue-500" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => handleDeleteResource(resource.id)} title="حذف">
  ...
  const BranchCard = ({
    ...
    return <Card className="mb-4">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <Button variant="outline" size="sm" className="text-xs flex items-center gap-1" onClick={() => {
        ...
        <CardContent>
          <div className="space-y-1">
            {resources.length > 0 ? resources.map(resource => <BranchResourceRow key={resource.id} resource={resource} />) : <div className="text-center py-3 text-gray-500">
      ...
      <Card className="border-company/20 glass">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <CardTitle className="text-2xl text-right"></CardTitle>
          </div>
        </CardHeader>
        <CardContent>
              ...
              <DialogContent className="sm:max-w-[500px]">
                ...
                <div className="mt-4">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {resourceTypes.map(type => <TableRow key={type.id}>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Input value={newResourceName} onChange={e => setNewResourceName(e.target.value)} placeholder="اسم المورد الجديد" className="text-right" />
                    </div>
                  </div>
                </div>
              </DialogContent>
            ...
            <DialogContent className="sm:max-w-[500px]">
              ...
              <div className="grid gap-4 py-4">
                <div>
                  ...
                  <select className="w-full p-2 border rounded-md text-right" value={selectedResourceType} onChange={e => {
                ...
                <div>
                  ...
                  <Input type="text" value={newResourceName} onChange={e => {
                  setNewResourceName(e.target.value);
                  setSelectedResourceType('');
                }} placeholder="اسم المورد الجديد" className="text-right" />
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-right mb-2">العدد المتوفر</label>
                    <Input type="number" value={formData.available} onChange={e => setFormData(prev => ({
                    ...prev,
                  ...
                  <div>
                    ...
                    <Input type="number" value={formData.in_use} onChange={e => setFormData(prev => ({
                    ...prev,
                    in_use: parseInt(e.target.value) || 0
                  }))} className="text-right" dir="rtl" />
                  </div>
                </div>
              </div>
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogContent className="sm:max-w-[500px]">
              ...
              <div className="grid gap-4 py-4">
                {editMode && <div className="text-right font-medium">
          ...
          {loading ? <div className="text-center py-10">
              ...
              {branches.length > 0 ? branches.map(branch => <BranchCard key={branch.id} branch={branch} />) : <div className="col-span-full text-center py-8">
